...
VSOut main(VSIn _in)
{
	...
	float4 worldPosition = mul(_in.Position, CB_Transform.world);
	...
	float3 vViewNormal = normalize(mul(float4(_in.Normal.xyz, 0.0f), CB_Transform.world).xyz);
	vViewNormal = normalize(mul(float4(vViewNormal, 0.0f), CB_Transform.view).xyz);
    
	float3 vViewTangent = normalize(mul(float4(_in.Tangent.xyz, 0.0f), CB_Transform.world).xyz);
	vViewTangent = normalize(mul(float4(vViewTangent, 0.0f), CB_Transform.view).xyz);
    
	float3 vViewBiNormal = normalize(mul(float4(_in.BiNormal.xyz, 0.0f), CB_Transform.world).xyz);
