...
namespace mh
{
	...
	eResult Animation3D::LoadFromFBX(const tFBXAnimClip* _clip)
	{
        ...
        mKeyFramesPerBone.reserve(_clip->vecBoneKeyFrame.size());
        for (size_t i = 0; i < _clip->vecBoneKeyFrame.size(); ++i)
        {
            mKeyFramesPerBone.push_back(tKeyFramesPerBone{});
            tKeyFramesPerBone& keyFramesPerBone = mKeyFramesPerBone.back();

            keyFramesPerBone.BoneIndex = _clip->vecBoneKeyFrame[i].BoneIndex;

            // 아래부터 키프레임 정보를 저장한다.
            keyFramesPerBone.vecKeyFrame.reserve(_clip->vecBoneKeyFrame[i].vecKeyFrame.size());
            for (size_t j = 0; j < _clip->vecBoneKeyFrame[i].vecKeyFrame.size(); ++j)
            {
                ...
                fbxsdk::FbxAMatrix		mat = _clip->vecBoneKeyFrame[i].vecKeyFrame[j].matTransform;
                ...
                keyFrame.Scale = float3((float)Scale.mData[0], (float)Scale.mData[1],
                    (float)Scale.mData[2]);
                keyFrame.Pos = float3((float)Pos.mData[0], (float)Pos.mData[1],
                    (float)Pos.mData[2]);
                keyFrame.RotQuat = float4((float)Rot.mData[0], (float)Rot.mData[1],
    ...
    bool Animation3D::CreateKeyFrameBuffer(const std::vector<tAnimFrameTranslation>& _vecAnimFrameTranslations)
    {
        MH_ASSERT(false == mKeyFramesPerBone.empty());
        ...
        vecFrameTrans.resize(mKeyFramesPerBone.size() * (size_t)m_FrameCount);
        for (size_t i = 0; i < mKeyFramesPerBone.size(); ++i)
        {
            for (size_t j = 0; j < mKeyFramesPerBone[i].vecKeyFrame.size(); ++j)
            {
                tKeyFrame& keyFrame = mKeyFramesPerBone[i].vecKeyFrame[j];
        ...
        m_KeyFrameBuffer->Create<tAnimFrameTranslation>(vecFrameTrans.size(), vecFrameTrans.data(), vecFrameTrans.size());
    }
}
